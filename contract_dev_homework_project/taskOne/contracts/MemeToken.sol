...
import "hardhat/console.sol"; //不需要安装依赖，只能在hardhat测试中运行
...
contract MemeToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        require(
            amount <= maxTxAmount,
            "Transfer exceeds max transaction limit"
        );

        //检查交易时间间隔，仅校验用户卖出时
        if (recipient == uniswapV2Pair) {
            require(
                block.timestamp >= _lastSellTime[sender] + COOLDOWN_SECONDS,
        ...
        if (tax > 0) {
            //将税费转入合约
            console.log("transfer tax to contract");
            super._transfer(sender, address(this), tax);
            _takeFeeAndLiquify(tax, isSell);
        }

        //给用户转账
        console.log("transfer recipientAmount to user");
            ...
            )
        {
            ...
            if (ethReceived > 0) {
                    ...
                    )
                {
                    ...
                    console.log("addLiquidityETH fail:",reason);
                    emit AddLiquidityFailure("AddLiquidity failed:", reason);
                } 
            }
        } catch Error(string memory reason) {
            console.log("swapExactTokensForETH fail:",reason);

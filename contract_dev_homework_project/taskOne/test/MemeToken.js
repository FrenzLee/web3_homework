...
const { abi: FactoryABI, bytecode: FactoryBytecode } = require('@uniswap/v2-core/build/UniswapV2Factory.json');
const { abi: RouterABI, bytecode: RouterBytecode } = require('@uniswap/v2-periphery/build/UniswapV2Router02.json');
const { abi: WETHABI, bytecode: WETHBytecode } = require('@uniswap/v2-periphery/build/WETH9.json');

describe("MemeToken (LMEME) Token", function () {
    let MemeToken, memetoken;
    let owner, user1, user2, marketingWallet;
    ...
    it("Should add initial liquidity successfully", async function () {
        ...
        //调用合约的 addInitialLiquidity 方法
        await expect(
            memetoken.addInitialLiquidity(tokenAmount, ethAmount)
        ).to.not.be.reverted;
    ...
    it("should collect 8% sell tax when user sells via router", async function () {
        ...
        // 用户（user1）通过 Router 买入代币
        const txBuy = await router.connect(user1).swapExactETHForTokens(
            0,
            pathBuy,
            user1.address,
            deadline,
        ...
        // 用户（user1）授权 Router 使用其代币
        await memetoken.connect(user1).approve(routerAddress, sellAmount);
        const txSell2 = await router.connect(user1).swapExactTokensForETHSupportingFeeOnTransferTokens(
            sellAmount,
            amountOutMin,
            pathSell,
            user1.address,
